...
public class Model : MonoBehaviour
{
    ...
    public bool IsValidMapPosition(Transform t)
    {
        foreach (Transform child in t)
        {
            ...
            Vector2 pos = child.position.Round();   //将三维向量转化为二维向量，且返回整数
            if (IsInsideMap(pos) == false) return false;    //判断block是否超出map边界
            if (map[(int)pos.x, (int)pos.y] != null) return false;    //判断该block上是否有物体
        }
        return true;
    }
